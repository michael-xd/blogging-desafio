...
describe('Users Reducer', () => {
  let initialState: fromUsers.State;
  let users: User[];

  beforeEach(() => {
    initialState = fromUsers.initialState;
    users = [
      {
        id: 5,
  ...
  describe('[Users] Load Users Success', () => {
    it('should update the state with users entities', () => {
  ...
  describe('[Users] Load Users Fail', () => {
    it('should update error in state', () => {
      const error = new Error('unexpected error');
      const action = loadUsersFailure({ error: error.message });
      const state = fromUsers.reducer(initialState, action);
